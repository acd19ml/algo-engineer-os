<!-- PROCESSED: 2026-05-13 → KNOWLEDGE/agent/agent-skills-closed-loop/（7 步闭环 mermaid + SKILLS_GUIDANCE prompt + 七道安全关卡 + 原子写入代码 + TOCTOU 反事实 + Skill 文件 frontmatter+body 形态 + 两层缓存 mermaid + cache_key 五元组 + L1/L2/L3 性能对比 + 强制措辞 system prompt + 条件激活 4 字段 + 渐进式加载 Tier 0-3 全部保留） -->
...
```mermaid
flowchart LR
    A["① 触发创建<br/>用户完成复杂任务<br/>5+ tool calls / 错误修复 / 用户纠正"]
    B["② Skill 创建<br/>skill_manager_tool.py<br/>Agent 调用<br/>skill_manage(action='create')<br/>验证 + 原子写入 + 安全扫描"]
    C["③ 索引构建<br/>prompt_builder.py<br/>扫描所有 SKILL.md 构建<br/>&lt;available_skills&gt; 紧凑索引<br/>注入 System Prompt"]

    D["④ 智能匹配<br/>每轮对话时 Agent 扫描索引<br/>System Prompt 强制要求：<br/>如果匹配，你必须加载"]
    E["⑤ 渐进式加载<br/>skills_tool.py<br/>Tier 0 → 分类<br/>Tier 1 → 元数据<br/>Tier 2 → 完整内容<br/>Tier 3 → 支撑文件"]
    F["⑥ 注入执行<br/>skill_commands.py<br/>作为 User Message 注入<br/>保护 Prompt Cache"]

    G{"执行中发现问题？"}

    H["⑦ 自动改进<br/>skill_manage(action='patch')<br/>Fuzzy Match 引擎<br/>原子替换 + 安全扫描 + 回滚保护"]

    I["任务完成"]

    J["⑧ 社区分享（可选）<br/>skills_hub.py<br/>发布到 agentskills.io<br/>其他用户安装后继续改进"]
...
```mermaid
flowchart TB
    A["收到新对话请求"] --> B{"Layer 1:<br/>进程内 LRU 缓存命中？"}

    B -- "命中" --> C["直接返回缓存的索引字符串<br/>耗时：约 0.001ms"]
    B -- "未命中" --> D{"Layer 2:<br/>磁盘快照有效？"}

    D -- "有效" --> E["读取 skills_prompt_snapshot.json<br/>反序列化预解析的元数据<br/>耗时：约 1ms<br/>写入 Layer 1 缓存"]
    D -- "无效 / 不存在" --> F["Layer 3: 全文件系统扫描<br/>递归遍历所有 SKILL.md<br/>解析每个文件的 YAML frontmatter<br/>耗时：约 50-500ms<br/>写入磁盘快照<br/>写入 Layer 1 缓存"]

    subgraph R["详细记录内容 & 架构亮点"]
        direction TB
        R1["缓存性能指标<br/>L1 LRU 延迟：约 0.001ms<br/>L2 快照延迟：约 1ms<br/>L3 全扫描延迟：50-500ms+"]
        R2["冷启动缓解策略<br/>L3 全件扫描为冷启动<br/>写入磁盘快照，将未来 L2 启动使用"]
        R3["增量元数据解析<br/>解析并写完元数据解析"]
        R4["序列化格式 JSON<br/>序列化格式 JSON，持久化存储"]
        R5["LRU 淘汰策略<br/>更出淘汰无法策略"]
    end
...
```mermaid
flowchart LR
    subgraph L["Skills 渐进式披露架构"]
        direction TB

        A["Tier 0: skills_categories()<br/>分类名 + 描述<br/>~50 tokens"]
        B["Tier 1: skills_list()<br/>name + description<br/>每个 Skill ~20 tokens"]
        C["Tier 2: skill_view(name)<br/>完整 SKILL.md 内容<br/>~500-5000 tokens"]
        D["Tier 3: skill_view(name, file_path)<br/>支撑文件内容<br/>references/、templates/<br/>scripts/、assets/"]
    ...
    subgraph R["详细内容记录"]
        ...
        R1["系统提示索引集成（Tier 0-1）<br/>• Tier 0 和 Tier 1 的分类名及技能名称集合进入系统提示引导<br/>• 索引用于初始发现，无需加载完整 Skill 内容<br/>• 只占用少量 tokens"]
        
        R2["Agent 按需调用（Tier 2-3）<br/>• Tier 2 和 Tier 3 需要 Agent 主动调用 skill_view() 工具<br/>• skill_view(name) 加载完整 SKILL.md 内容<br/>• skill_view(name, file_path) 加载支撑文件"]
        
        R3["架构核心理念与性能<br/>• 核心理念：按需加载，逐步深化<br/>• 减少系统提示中的初期 token 负载<br/>• 提高响应性<br/>• 节省 API 成本"]
    end

    A -. "索引进入 System Prompt" .-> R1
    C -. "按需查看完整 Skill" .-> R2
    D -. "进一步查看支撑文件" .-> R2
    R2 --> R3
```
...
```mermaid
flowchart TB
    subgraph A["方案 A：修改 System Prompt（已否决）"]
        direction TB
        A1["Turn 1: System Prompt<br/>(2K tokens) → Cache MISS → 全量处理"]
        A2["加载 Skill → 修改 System Prompt<br/>(5K tokens)"]
        A3["Turn 2: System Prompt<br/>(5K tokens) → Cache MISS → 全量处理"]
        A4["Turn 3-30: 每轮都 Cache MISS"]
        A5["30 轮对话总成本：约 150K tokens 处理量"]
    end

    subgraph B["方案 B：User Message 注入（采用）"]
        direction TB
        B1["Turn 1: System Prompt<br/>(2K tokens) → Cache MISS → 缓存"]
        B2["加载 Skill → 作为 User Message 追加"]
        B3["Turn 2: System Prompt<br/>(2K tokens) → Cache HIT → 跳过"]
        B4["Turn 3-30: 每轮都 Cache HIT"]
        B5["30 轮对话总成本：约 7K tokens 处理量<br/>节省 95%+"]
    end

    subgraph C["详细记录：Token 使用分析"]
        direction TB
        C1["方案 A 基础分解<br/>Turn 1: 2K tokens<br/>Turn 2: 5K tokens<br/>Turn 3-30: 28 * 5K = 140K tokens<br/>Total: ~150K"]
        C2["方案 B 基础分解<br/>Turn 1: 2K tokens<br/>Turn 3-30: Cache HIT，仅处理 User Message + Skills 增量<br/>Total: ~7K"]
        C3["方案 B 核心优势<br/>缓存 System Prompt，避免全量处理<br/>大幅降低计算量和成本<br/>提高响应速度"]
    end
...
flowchart LR
    ...
    A --> B["数据外泄<br/>检测目标：数据或密钥泄露<br/>严重程度：Critical<br/>示例模式：--config / .data / env"]
    A --> C["Prompt 注入<br/>检测目标：在 Prompt 注入后，让程序进入 Prompt 注入<br/>严重程度：Critical<br/>示例模式：prompt_injection"]
    A --> D["破坏性操作<br/>检测目标：使用破坏性操作导致文件或系统删除<br/>严重程度：Critical<br/>示例模式：rm / environment"]
    A --> E["破坏性操作<br/>检测目标：操作可能破坏性操作，并拦截不能操作的行为<br/>严重程度：Critical<br/>示例模式：--asator / --otype"]
    A --> F["持久化<br/>检测目标：使用安全检测准备案的持久化<br/>严重程度：Critical<br/>示例模式：--propose"]
    A --> G["反向 Shell<br/>检测目标：对接受安全检测的发生中直接地图的反向 Shell<br/>严重程度：Critical<br/>示例模式：/in/wai"]
    A --> H["代码混淆<br/>检测目标：安全检测代码混淆，不使混淆代码绕过<br/>严重程度：Critical<br/>示例模式：base64 / obfuscation"]
    A --> I["供应链攻击<br/>检测目标：假装供应链或依赖链攻击，以免供应链优化<br/>严重程度：Critical<br/>示例模式：const / end / port"]
    A --> J["凭证暴露<br/>检测目标：安全场景的数据、凭证、账号泄露<br/>严重程度：Critical<br/>示例模式：password required"]
    A --> K["凭证暴露<br/>检测目标：安全策略缺失凭证泄露，或符号用户注入<br/>严重程度：Critical<br/>示例模式：token / key / secret"]
    A --> L["路径穿越<br/>检测目标：安全检测入侵路径，路径穿越，路径穿越<br/>严重程度：Critical<br/>示例模式：/path / ../"]
    A --> M["隐形 Unicode<br/>检测目标：隐形容易触碰的隐形、能影响能力的隐形 Unicode<br/>严重程度：Critical<br/>示例模式：Invisible Unicode"]
```
...
flowchart LR
    ...
    subgraph M["Memory"]
        direction TB
        M1["陈述性知识<br/>(Declarative)"]
        M2["用户偏好、环境信息、工具怪癖"]
        M3["用户偏好 TypeScript<br/>服务器运行 Ubuntu 22.04"]
        M4["MEMORY.md / USER.md"]
        M5["System Prompt<br/>(始终存在)"]
        M6["低<br/>(稳定事实)"]
        M7["不可共享<br/>(个人偏好)"]
    end
    ...
    subgraph S["Skill"]
        direction TB
        S1["程序性知识<br/>(Procedural)"]
        S2["操作步骤、命令序列、陷阱清单"]
        S3["用 Axolotl 微调 LLM 的完整流程"]
        S4["SKILL.md + 支撑文件"]
        S5["User Message<br/>(按需加载)"]
        S6["高<br/>(每次使用可能 patch)"]
        S7["可通过 Skills Hub 共享"]
    end
...
```mermaid
flowchart LR
    subgraph V["Voyager (NVIDIA, 2023)"]
        direction TB
        V1["单 Agent，单环境<br/>(Minecraft)"]
        V2["Skills 存储在内存 dict<br/>(进程重启即丢失)"]
        V3["无安全考量<br/>(封闭环境)"]
        V4["无社区分发<br/>(单用户)"]
        V5["直接注入 prompt<br/>(无成本优化)"]
        V6["JavaScript 函数<br/>(代码即技能)"]
    end
    ...
    subgraph H["Hermes Agent (Nous Research, 2024-2025)"]
        direction TB
        H1["多平台 Agent<br/>(CLI / Telegram / Discord / Slack...)"]
        H2["文件系统持久化 + 两层缓存<br/>(跨进程 / 跨重启)"]
        H3["90+ 威胁模式扫描 + 信任分级<br/>(面向互联网)"]
        H4["Skills Hub (agentskills.io)<br/>(社区生态)"]
        H5["User Message 注入<br/>(保护 Prompt Cache)"]
        H6["YAML + Markdown<br/>(结构化元数据 + 自然语言指令)"]
    end
