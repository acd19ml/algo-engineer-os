<!-- PROCESSED: 2026-05-13 → KNOWLEDGE/agent/memory-architecture-thesis/（三件套 + System 1/2 + JitRL + 三类瓶颈 + bi-temporal + memory tool 工具化已全部蒸馏） -->
...
具体来说，Zep 会对图谱中的边（Edge）加入时间标注（Temporal Validity）。例如$(\text{User}, \text{is\_CEO\_of}, \text{Company\_A})$不再是一个静态事实，而是带有一个有效时间窗口（如$[t_{start}, t_{end}]$），用来区分"曾经为真"和"当前为真"。
...
另外，MAGMA 论文（2025）提出了四正交图架构（语义、时间、因果、实体），其中时间图的设计也值得参考。MAGMA 的时间图$\mathcal{E}_{temp}$定义为严格有序对$(n_i, n_j)\quad(\tau_i < \tau_j)$，构成不可变时间骨干；快速路径只建时间边，慢速路径异步推断因果/实体等高阶关系。消融实验显示移除时间骨干后评判分数从 0.700 降至 0.647，证明时间排序提供了不可替代的推理轴。
...
让$A_{est}$看到"时间结构特征"，而非只看到"老不老"：对同一 fact_key（或同一类经验轨迹）可抽取结构化时间信号进入优势估计：
...
北京
5月5日
引入时间维度，指出现在LLM的参数化的问题是静态化。却有洞见，独开个局面。信息有两类，一类在时间这个维度上是无方向的，比如类似数学等科学。还有一类有明显的时间约束，比如历史“事实”，某个语境下的“叙述、决策”等，这类信息不做时间区域的遮掩，往往得到的结论和模式都是事后诸葛亮时的错误认知和无用经验。实际上不只是模型容易犯这种错误，更是大多数人，即便是领域的专家也经常在这样的问题上得出错误的看法。这一点在推理上尤其突出。后来者由于对过往信息的全部掌握，和当事者完全不同。机器能记住阅读和使用大量信息，是一个巨大优势，但也就脱离了个体化的体验和视角。在很多情境下“不知道”才能接近真实，也很重要。在另外一个更广阔的领域，也许它比知道更有用。

.malog
广东
4月20日
这篇确实很有深度，有挺多地方还是比较难懂的
已无更多数据
