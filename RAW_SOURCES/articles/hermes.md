<!-- PENDING: 2026-05-13 — 内容是阿里云 Hermes 可观测插件（process observability for ReAct loops），讲怎么把 Agent 的多轮推理 / 工具调用 / token / 时延 / 错误还原为结构化 trace。与现有 KNOWLEDGE 节点重叠少，但话题相对边缘（产品向不是范式向）。建议下次若做"Agent 可观测性"主题节点，回头读：可观测 4 类问题（过程不可见 / 成本归因 / 工具调用真伪 / 数据泄露）+ 调用链结构化 + Token 归因方法。本次未蒸馏。 -->
...
江苏
4月27日
AgentScope开发的Agent应用支持么

阿里云云原生
作者
4月27日
自建应用的灵活性比较大，这里推荐使用 arms 探针来进行可观测，agentscope 框架是支持的，https://help.aliyun.com/zh/cms/cloudmonitor-2-0/connecting-to-an-agentscope-python-application 可以参考该文档来进行可观测接入， 后续如果文档无法解决您的疑问，欢迎通过钉钉答疑群（群号：159215000379）与我们联系。
...
作者
4月27日
可观测支持openclaw的，接入方式也是类似，可以参考：当你的 Agent 会“多轮思考”，Trace 却还停留在单轮：阿里云 CMS OpenClaw 可观测插件升级
已无更多数据
