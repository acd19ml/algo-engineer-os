...
>
> 范围：5 个角色 agent 的 prompt 主体 + JSON 输出 schema + Dify 工作流图 + MCP 工具集 + ReAct 优化路径 + 真实数字 20% → 70% 是怎么迭代上来的。
>
...
Agent 子系统基于 **Dify 平台**构建分层智能体工作流，对外接收告警卡片触发 / 用户自然语言提问。
...
Dify 工作流引擎编排 MCP 工具调用逻辑，**利用 ReAct 模式对复杂问题自行拆解和迭代，多次改写问题并多次执行工具调用从而生成质量更优的结果**。
...
**Baseline**：MVP 早期 demo 根因定位成功率约 **20%**（Mock 系统数据，非生产 A/B）。

**优化措施**（按时间顺序逐步迭代上来的）：

| # | 措施 | 解决什么 |
|---|---|---|
| 1 | **开发集成 CMDB 服务** | 让大模型理解系统架构上下文，不再"瞎猜根因" |
| 2 | **拆分知识库**（系统架构 / 历史故障案例 / 运维流程 三类） | 按问题类型有选择启用相关知识库，提高检索精度 |
| 3 | **动态注入上下文** | 不再静态长上下文，按需 MCP 实时查 → 信息时效性 + 上下文精简 |
| 4 | **时间通过中心化函数统一** | 解决"模型自己心算时间出错"的硬伤——所有时间处理走工具 |
| 5 | **重写提示词结构** | 5 个 agent 的 prompt 都做了"工作流程严格 5 步 / 限制 / 禁止行为 / JSON schema"的结构化重写 |
| 6 | **加入缓存机制** | 重复查询不再重新走完整 MCP 链路 |
| 7 | **ReAct 工程优化 4 件事**（见 §3.3） | 上下文压缩 + 智能终止 + 显式总结 + 减少无效循环 |

**当前迭代终点**：约 **70%** 成功率（Mock 系统 demo 数据）。
...
## 8. ClaudeCode vs Dify 对比与选型（项目结束时做的事后对比）

为测试模型效果对比了论坛火的 ClaudeCode 实现。**ClaudeCode 是高度抽象的通用型框架，200 多行代码即可完成具有上下文管理 / 工具调度 / 模型规划的完整智能体**。其内置多个极致优化的工具和提示词使任务执行异常高效。

### 8.1 8 个维度对比

| 维度 | ClaudeCode 方案 | Dify 方案 |
|---|---|---|
| **开发效率** | 需编写核心逻辑代码（部分由 Claude 生成） | 可视化编排，快速搭建原型 |
| **灵活性** | 可深度定制算法 / 模型 | 依赖 LLM 能力，复杂逻辑需结合自定义函数 |
| **准确性** | 确定性算法 + AI 模型，内置多种优化算法 | LLM 可能产生幻觉，需提示词参数等方式优化 |
| **实时性** | 微秒级响应（模型推理） | 秒级延迟（LLM API 调用） |
| **运维成本** | 需维护训练管道 / 部署环境 | 无基础设施管理，SaaS 化运维 |
| **可观测支持** | 需额外开发日志 / 追踪分析模块 | 原生支持文本 / 日志分析 + 调用量统计 |
| **知识沉淀** | 需独立构建知识库 | 内置知识库自动学习 |
| **典型场景** | 金融互联网等低延迟算法控制场景 | 零售电商企业快速上线场景 |

### 8.2 选型结论

> 需求简单 / 快速上线优先用 **Dify**；需要控制算法 / 超低延迟推荐 **ClaudeCode**；复杂场景下 **80% 标准场景 + 20% 定制可结合**。

**项目实际选 Dify** 的理由（再强调一遍，决策详细见 `README.md`"关键决策"段）：

- **2025.07 时 ClaudeCode 还没出**——这个对比是项目做完之后的事后回看
- **当时团队需要快速搭建可演示原型 + 非算法成员要能理解和接手**——Dify 可视化编排是最适合的工具
- **MVP 阶段优先工程优化、不做模型层优化**——Dify 工作流引擎已经够用
...
- **为什么选 Dify 而不是 agent loop / 时代背景 / 复盘**  → 见 `README.md` 时代背景 / 真实选型路径 / 复盘段
