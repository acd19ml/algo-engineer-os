...
> 七牛云 1024 实训营 ZeroOps 智能运维项目（CEO 许式伟亲带 + mentor 王昶敏带教）。我作为项目组长 + Agent 设计负责人协调 6 人推进，核心交付是"指标下钻分析"子系统——基于 Dify 工作流编排的 L1/L2/L3 多智能体 + MCP 工具集成。
...
| [`agent-subsystem.md`](./agent-subsystem.md)（Agent 子系统解剖） | Dify 工作流编排 + 模型选型 / ReAct PDL 改造 / MCP 工具集 schema / 5 个角色 agent 的 prompt 主体 + JSON 输出 schema / 4 类知识库设计 / 20%→70% 优化路径 / ClaudeCode vs Dify 对比 | 面试官追问"你 agent 怎么设计的"时打开 |
...
- **多智能体根因分析下钻子系统（我的核心交付）**：基于 Dify 平台编排 L1（提问专家：文档检索 + 问题类别 + 决策）/ L2（专门 agent：日志查询分析 / Metric 指标查询分析 / Tracing 查询分析）/ L3（通用专家 + 语言专家）的三层 agent 协作架构。所有工具通过 MCP Server 封装在函数计算上动态调用（日志 / 指标 / 链路 / CMDB 拓扑）
...
## 真实的技术选型路径（不是"naive 选了 Workflow"）

项目初期，我**先试过 agent loop 编排**——类 coding agent 式 / human-in-the-loop 风格的实现。

**结果**：**调试困难**。原因（事后反思）：

1. 当时 agent loop 没有成熟的开源参考实现可借鉴
2. 多 agent 协同的失败模式（trajectory 错误传播、context 累积、循环不终止等）没有标准化的可观测性工具
3. 团队需要快速搭建可演示原型，且非算法成员要能理解和接手 → agent loop 黑盒程度太高

**取舍**：**主动从 agent loop 退到工作流模式**。在 Dify 上做**可视化工作流编排** + 每个节点单独可调试 + 每个 agent 的输入输出显式记录。

这是有意识的 ROI 取舍——**用工作流的可调试性换 agent loop 的灵活性**，是当时技术坐标下的合理选择。
...
| **Agent 编排范式**（最容易被追问） | **(a) Agent loop（类 coding agent）/ (b) DAG 工作流（DeepResearch 式）/ (c) Dify 可视化工作流 + ReAct** | **(c) Dify 工作流 + ReAct** | **先试了 (a) → 调试困难放弃**。2025.07 时市面无开源 agent loop 系统可参考，团队需要快速做可演示原型 + 非算法成员要能接手。**用工作流可调试性换 agent loop 灵活性，是当时技术坐标下的合理 ROI 取舍** |
| Agent 编排平台 | LangChain / LangGraph / Dify / 自建 | **Dify on K8s** | 工作流可视化便于团队（含非算法成员）理解 / 内置 ReAct 模式 / 支持节点级 prompt 调试 / MCP 工具集成成熟 |
...
**情境**：基于 Dify 的 L1/L2/L3 多智能体根因分析模块单独可以跑通，**但因为产品设计反复调整 + Mock 系统接口持续变更 + 时序检测数据格式不稳定，端到端主链路始终联调不顺**。

**坦诚结果**：**Agent 模块没有真正接入主链路完整跑通**——三个月内 Agent 单独的根因分析逻辑设计完整、Dify 工作流配通、MCP 工具调用通了，**但与外部告警 / 体检中心 / 发布系统的串联停在 Mock 阶段**。
...
- ✅ **多智能体根因分析下钻模块独立 demo 跑通**——5 个角色 agent + Dify 工作流 + MCP 工具集成 + ReAct 迭代收敛
...
我做的 Dify 工作流 + ReAct + Result Fusion 多智能体在 **2026 年回看**会被人问"为什么不用 coding agent / agent loop / Skills / Harness"——但这些概念在 **2025.07** 都还没成型。

**反思**：我**主动从 agent loop 退到工作流**是当时技术坐标下的合理 ROI 取舍——用工作流的可调试性换 agent loop 的灵活性，特别是非算法成员需要能接手的情况下。

**面试金句**：

> 这个项目在 2025 年 7 月开始时，市面上还没有开源的 agent loop 系统，上下文工程刚兴起，最火的是基于 DAG 的 DeepResearch。我项目初期试过编排 agent loop 类 coding agent 式 / human-in-the-loop，但调试困难——团队需要快速搭建可演示原型，非算法成员要能接手。所以**主动退到工作流模式**。这不是我对 agent loop 的能力不了解，是当时的合理取舍。今天回看 coding agent / Skills / Harness 都涌现了，但我做的设计仍能挂在 OpsAgent + Result Fusion 这条主流方向上。

**下次实操**：技术选型决策要写时间戳——"这个选型是 X 时刻基于 Y 技术坐标做出的"。给未来的自己和面试官留一个时代背景的锚点。
...
- 负责"指标下钻分析"AI 子系统的设计与实现：基于 Dify 工作流编排的 **L1（任务规划 + 类别判断 + 决策）/ L2（日志 / 指标 / Tracing 三个专门 agent）/ L3（通用专家 + 语言专家）三层多智能体架构**，通过 **MCP Server（部署在函数计算）** 标准化封装日志 / 指标 / 链路 / CMDB 拓扑等工具，使大模型按需动态查询多源运维数据。
...
- `CAREER/interview-bank/technical/qiniu-agent-loop-vs-workflow.md`——为什么选 Dify 工作流而不是 agent loop
