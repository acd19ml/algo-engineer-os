...
- **挑战防御 / GAP 追踪**：[`interview-defense-matrix.md`](./interview-defense-matrix.md)（catalog + readiness + GAP）
- **单题深答** [`interview-answers/`](./interview-answers/)：
  - `qiniu-agent-loop-vs-workflow.md`——为什么选 LangGraph StateGraph 而不是自由 agent loop
  - `qiniu-multi-agent-decomposition.md`——为什么这样拆 agent + 对照 Flow-of-Action
  - `qiniu-multimodal-fusion-paradigm.md`——Result Fusion vs Model Fusion vs Feature Fusion
  - `qiniu-opsagent-vs-agentops.md`——OpsAgent vs AgentOps + 项目缺什么
  - `qiniu-react-context-management.md`——ReAct 三痛点工程优化
  - `prompt-caching-agent-loop.md`——Prompt Caching 机制 + 连回项目

> 行为题 STAR 故事原在 `CAREER/interview-bank/behavioral/`，已随面试卡退场；叙事素材在本 README 决策复盘 + `interview-defense-matrix.md`。
